...
// 점수 점멸 기능을 위한 변수
int scoreDisplayTimer = 0; // 점수 표시 타이머
int flashTimer = 0;        // 점멸 타이머
int flashState = 1;        // 점수 표시 상태 (1: 표시, 0: 숨김)
...
int main(void) {
    ...
    while (1)
    {
        ...
        while (1) { // 반복 실행
            ...
            // 점수가 50의 배수일 때 화면에 표시하는 코드 추가 위치
            if (score % 50 == 0 && score > 0 && scoreDisplayTimer == 0) {
                scoreDisplayTimer = 20; // 약 2초 동안 표시 (루프 속도에 따라 조정)
                flashTimer = 0;         // 점멸 타이머 초기화
                flashState = 1;         // 점수 표시 활성화
            }

            if (scoreDisplayTimer > 0) {
                flashTimer++; // 점멸 타이머 증가
                if (flashTimer >= 5) { // 점멸 주기 조정 (5 루프마다 토글)
                    flashState = !flashState; // 표시/숨김 상태 변경
                    flashTimer = 0; // 타이머 리셋
                }
                ...
                if (flashState) {
                    ...
                    // 화면 상단 가운데 점수 표시
                    int row = 1; // 화면 상단
                    int col = (WIDTH - strlen(scoreDisplay)) / 2;
                    for (int i = 0; i < strlen(scoreDisplay); i++) {
                        screen[row][col + i] = scoreDisplay[i];
                    }
                }

                scoreDisplayTimer--; // 표시 타이머 감소
            }
            clearScreen();
            score += 1;
            printScreen(); // 화면 출력
